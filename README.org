...
** 椭圆曲线的一般形式
椭圆曲线方程: y^{2} = x^{3} + a * x + b, 其中 a, b 为指定常数 (貌似还有一些特殊限制, 不过现在用不上)

注: 一般教程的配图都是根据这个方程画的, 和实际密码学中使用的有很大区别, 牵强附会的解释会对初期理解椭圆曲线造成很大的麻烦

** 密码学中的椭圆曲线
椭圆曲线是连续的, 不好用于加密解密, 考虑把椭圆曲线定义到有限域 F_{p} 上:

y^{2} \equiv x^{3} + a * x + b (mod p), 其中 p, a, b 为指定常数, x \in F_{p}, y \in F_{p}

注: ECC 上的点, 我们只实现了加法, 只是一个群 (阿贝尔群); 但是它又分为 x 和 y 坐标, 均为 F_{p} 域内的点, 在不引起混淆的情况下, 通常也会称为有限域内的点
...
其中单个点的情况是 y1 = 0, 这时它的逆元就是它自己, x 满足 x^{3} + a * x + b \equiv 0 (mod p)
...
注1: P1 != P2 时, 还是有可能 x2 = x1 的, 这时其实就是负元相加, 为零元 (代码实现时需要注意)

注2: P1 = P2 是, 如果有 y1 = 0, 那么计算 lambda 时也会出错, 但这是不可能的, 根据上一节的注2, 我们可以知道必有 P1 = P2

** 减法 (验证负元的定义是否正确)
P(x3, y3) = P(x1, y1) + P(x2, y2)

若 P2 为 P1 的负元, 那么 P3 就是零元, 所以 P(x3, y3) - P(x1, y1) = P(x1, p - y1) 是 P1 的负元, 即 P2, 符合

若 P2 为零元, 那么 P(x3, y3) - P(x1, y1) = P(x1, y1) + P(x1, p - y1)
